...
payload = ("select data in ('" + s_time + "','" + e_time + "') "
           "limit 200000 "
           "where Metadata/LoadLocation/Building='Girls Hostel'"
           " and Metadata/LoadLocation/Wing='" + wing + "'"
           " and Metadata/Extra/PhysicalParameter='Power'"
           " and Metadata/Instrument/SupplyType='Power'"
           " and Metadata/Instrument/MeterID='" + meterid + "'"
           " and Metadata/LoadLocation/Floor='" + floor + "'")
print payload
...
df1 = (
    pd.DataFrame(
        np.zeros(
            (len(readings), 2)), columns=['time', 'power']))
...
for i in range(1, 4):
    ...
    payload = ("select data in ('" + s_time + "','" + e_time + "') "
               ...
               "where Metadata/LoadLocation/Building='Girls Hostel'"
               " and Metadata/LoadLocation/Wing='" + wing + "'"
               " and Metadata/Extra/PhysicalParameter='PowerPhase" + str(i) + "'"
               " and Metadata/Instrument/SupplyType='Power'"
               " and Metadata/Instrument/MeterID='" + meterid + "'"
               " and Metadata/LoadLocation/Floor='" + floor + "'")
    print payload
    ...
    df_tmp = pd.DataFrame(
        {'time': readings[:, 0] / 1000, 'powerphase' + str(i): readings[:, 1]},
        columns=['time', 'powerphase' + str(i)])

    # Merge the dataframes - this takes care of missing values
    df1 = pd.merge(df1, df_tmp, on='time', how='outer')

    # df1['powerphase' + str(i)] = p = readings[:, 1]
    # power.append(p)

# Replace the NaN values with previous row's value
df1 = df1.sort('time')
df1 = df1.reset_index(drop=True)
df1 = df1.fillna(method='pad')

tp = np.array([datetime.datetime.fromtimestamp(x) for x in df1.time])
...
df2 = (pd.DataFrame(np.zeros((len(readings), 2)),
       columns=['time', 'lightpower']))
...
for i in range(1, 4):
    ...
    df_tmp = pd.DataFrame(
        {'time': readings[:, 0] / 1000, 'lightphase' + str(i): readings[:, 1]},
        columns=['time', 'lightphase' + str(i)])
    df2 = pd.merge(df2, df_tmp, on='time', how='outer')
...
tl = np.array([datetime.datetime.fromtimestamp(x) for x in df2.time])
